...
if __name__ == '__main__':
    ...
    baseline_model = RandomForestRegressor(n_estimators=100, random_state=SEED)
    ...
    from pathlib import Path
    save_path = Path("saved_models") / TICKER / "rf_reg_tuned"
    save_path.mkdir(parents=True, exist_ok=True)
    with open(save_path / f"{TICKER}_RF_baseline_comparison.json", "w") as f:
        json.dump(baseline_metrics, f, indent=4)

    save_dir = Path("saved_models") / TICKER / "rf_reg_tuned"
    ...
    price_fig.add_trace(go.Scatter(
        x=plot_test.index, y=plot_pred, mode="lines", name="Tuned RF Predicted",
    ...
    price_fig.update_layout(
        title="Tuned Random Forest: Predictions vs Actual Price Action (Last 6 Months)",
        xaxis_title="Date", yaxis_title="Price (Rs)",
        template="plotly_white", xaxis_rangeslider_visible=False,
    )
    price_fig.write_html(f"plot_regression_tuned_candlestick_{TICKER}.html")
    ...
    scatter_fig.update_layout(
        title="Tuned Random Forest: Predicted vs Actual",
        xaxis_title="Actual Price (Rs)", yaxis_title="Predicted Price (Rs)",
        template="plotly_white",
    )
    scatter_fig.write_html(f"plot_regression_tuned_scatter_{TICKER}.html")
